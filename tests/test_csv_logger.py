...
def test_appends_timestamp_value(tmp_path: Path) -> None:
    ...
    log.log("engine_rpm", timestamp=datetime(2026, 6, 4, 12, tzinfo=UTC).timestamp(), value=2150.0)
    log.log(
        "engine_rpm",
        timestamp=datetime(2026, 6, 4, 12, 0, 1, tzinfo=UTC).timestamp(),
        value=2160.0,
    )
    log.close()
...
def test_rotates_at_day_boundary(tmp_path: Path) -> None:
    log = CSVSignalLogger(base_dir=tmp_path)
    day1 = datetime(2026, 6, 4, 23, 59, 59, tzinfo=UTC).timestamp()
    day2 = datetime(2026, 6, 5, 0, 0, 1, tzinfo=UTC).timestamp()
    log.log("x", day1, 1.0)
    log.log("x", day2, 2.0)
    log.close()
