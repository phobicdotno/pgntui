...
### Changed
- About dialog is wider (80 cols, capped at 95% of the screen) and its
  changelog entries are aligned in a version column and kept to one line each,
  so nothing wraps.
- README "Status" reworded in plain language; Hotkeys list adds `C`
  (Connection) and `A` (About).
